...
def animate(i):
    ...
    data_phase_pos1 = f[f"particle_{particle_type1}/pos{j}"]
    x1 = data_phase_pos1[:, 0]
    y1 = data_phase_pos1[:, 1]

    data_phase_vel1 = f[f"particle_{particle_type1}/vel{j}"]
    vx1 = data_phase_vel1[:, 0]
    vy1 = data_phase_vel1[:, 1]


    data_phase_pos2 = f[f"particle_{particle_type2}/pos{j}"]
    x2 = data_phase_pos2[:, 0]
    y2 = data_phase_pos2[:, 1]

    data_phase_vel2 = f[f"particle_{particle_type2}/vel{j}"]
    vx2 = data_phase_vel2[:, 0]
    vy2 = data_phase_vel2[:, 1]

    ax_phase1.clear()
    
    ax_phase1.scatter(x1, y1, marker='o', color='b', alpha= 1.0, s = 2 , label=f"{particle_type1} phase space")
    ax_phase1.scatter(x2, y2, marker='o', color='r', alpha= 1.0, s = 2 , label=f" {particle_type2} phase space")
    ax_phase1.set_xlabel('$x$')
    ax_phase1.set_ylabel('$v$')
    ax_phase1.legend(loc='upper right', framealpha=0.5)
