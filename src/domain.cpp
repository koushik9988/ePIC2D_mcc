...
Domain::Domain(double x0, double y0, double dx, double dy, int nx, int ny ):x0(x0), y0(y0), dx(dx), dy(dy), nx(nx), ny(ny)
{
    ...
    dirichlet_nodes = Matrix<bool>(nx,ny);
    dirichlet_nodes = false;
    Lx = (nx-1)*dx;
    Ly = (ny-1)*dy;

    enable_elastic_collision = false;
    enable_excitation_collision = false;
    enable_ionization_collision = false;
    enable_pion_elastic = false;
    enable_nion_elastic = false;
    enable_e_detach_collision = false;
    GAS_DENSITY = 0.0;
}
...
void Domain::display(vector<Species> &species)
{
    ...
    for(Species &p : species)
    {
        ...
        cout << "\n  Posistion Initialization Type:    " << p.initialization_posx;
        cout << "\n  Posistion Initialization Type:    " << p.initialization_posy;
        cout << "\n  Velocity Initialization Type:    " << p.initialization_velx;
        cout << "\n  Velocity Initialization Type:    " << p.initialization_vely;
        cout << "\n--------------------------------------------\n";
        index++;
    }
...
void Domain::ComputeRho(vector<Species> &species)
{
    ...
    for (int i = 0; i < nx_local; i++)
    {
        for (int j = 0; j < ny_local; j++)
        {
            ...
            for (int s = 0; s < nsp; s++)
            {
                sum += species[s].den(i,j) * (species[s].charge * (1/Const::QE));
            }
            rho(i,j) = sum;
        }
    }
...
/*
vec<double> Domain::Compute_PE(Species &species)
{
    vec<double> pe(2);
	...
	for (int i=0; i < nx; i++)
	{
        for (int j=0;j<ny;j++)
        {
            double Ex2 = efx(i,j)*efx(i,j);
            double Ey2 = efy(i,j)*efy(i,j);
            pe(0) += Ex2*(L*L);
            pe(1) += Ey2*(L*L);

        }		
	}
	pe(0) = pe(0)*0.5*Const::EPS_0*unnorm*unnorm;
    pe(1) = pe(1)*0.5*Const::EPS_0*unnorm*unnorm;
...
vec<double> Domain::Compute_PE(Species &species)
{
    ...
    #pragma omp parallel for collapse(2) reduction(+:sum_ex,sum_ey) schedule(static)
    for (int i = 0; i < nx; i++)
    {
        for (int j = 0; j < ny; j++)
        {
    ...
    pe(0) = sum_ex * 0.5 * Const::EPS_0 * unnorm * unnorm;
    pe(1) = sum_ey * 0.5 * Const::EPS_0 * unnorm * unnorm;
