...
void Output::write_grid_metadata(const std::vector<GridMeta>& grids_meta)
{
    Group metadata = file.openGroup("/metadata");
    Group grids_group;

    if (metadata.exists("grids"))
    {
        grids_group = metadata.openGroup("grids");
    }
    else
    {
        grids_group = metadata.createGroup("grids");
    }

    int grid_count = static_cast<int>(grids_meta.size());
    grids_group.createAttribute("grid_count", PredType::NATIVE_INT, DataSpace(H5S_SCALAR)).write(PredType::NATIVE_INT, &grid_count);

    StrType str_type(PredType::C_S1, H5T_VARIABLE);
    DataSpace scalar_space(H5S_SCALAR);

    for (size_t i = 0; i < grids_meta.size(); i++)
    {
        const GridMeta& gm = grids_meta[i];
        std::string group_name = "grid_" + std::to_string(i);
        Group g = grids_group.createGroup(group_name);

        Attribute type_attr = g.createAttribute("type", str_type, scalar_space);
        type_attr.write(str_type, gm.type);

        g.createAttribute("min_x", PredType::NATIVE_INT, DataSpace(H5S_SCALAR)).write(PredType::NATIVE_INT, &gm.min_x);
        g.createAttribute("min_y", PredType::NATIVE_INT, DataSpace(H5S_SCALAR)).write(PredType::NATIVE_INT, &gm.min_y);
        g.createAttribute("max_x", PredType::NATIVE_INT, DataSpace(H5S_SCALAR)).write(PredType::NATIVE_INT, &gm.max_x);
        g.createAttribute("max_y", PredType::NATIVE_INT, DataSpace(H5S_SCALAR)).write(PredType::NATIVE_INT, &gm.max_y);

        g.createAttribute("center_x", PredType::NATIVE_INT, DataSpace(H5S_SCALAR)).write(PredType::NATIVE_INT, &gm.center_x);
        g.createAttribute("center_y", PredType::NATIVE_INT, DataSpace(H5S_SCALAR)).write(PredType::NATIVE_INT, &gm.center_y);
        g.createAttribute("num_electrodes", PredType::NATIVE_INT, DataSpace(H5S_SCALAR)).write(PredType::NATIVE_INT, &gm.num_electrodes);
        g.createAttribute("grid_rad", PredType::NATIVE_DOUBLE, DataSpace(H5S_SCALAR)).write(PredType::NATIVE_DOUBLE, &gm.grid_rad);
        g.createAttribute("electrode_rad", PredType::NATIVE_DOUBLE, DataSpace(H5S_SCALAR)).write(PredType::NATIVE_DOUBLE, &gm.electrode_rad);
        g.createAttribute("voltage", PredType::NATIVE_DOUBLE, DataSpace(H5S_SCALAR)).write(PredType::NATIVE_DOUBLE, &gm.voltage);
    }
}

void Output::write_grid_mask()
{
    Group metadata = file.openGroup("/metadata");

    hsize_t nx = domain.nx;
    hsize_t ny = domain.ny;
    hsize_t dims_mask[2] = {ny, nx};
    DataSpace dataspace_mask(2, dims_mask);

    H5::DataSet dataset_mask = metadata.createDataSet("grid_mask", H5::PredType::NATIVE_UCHAR, dataspace_mask);
    ...
    dataset_mask.write(mask.data(), H5::PredType::NATIVE_UCHAR);
}
