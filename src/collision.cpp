...
void CollisionHandler::collision_electron(double xe, double ye, double &vxe, double &vye, double &vze, int eindex, Species &species1, Species &species2)
{
    ...
    {  
        ...
        species2.AddParticle(Particle(xe, ye, Init::SampleVel(species2,species2.temp),  Init::SampleVel(species2,species2.temp),  Init::SampleVel(species2,species2.temp)));
    }
...
void CollisionHandler::handle_collisions(Species &electron, Species &target_gas)
{
    for (Particle &part : electron.part_list)
    {
        ...
        if (p_coll > rnd())
        {
            collision_electron(part.pos[0],part.pos[1], part.vel[0], part.vel[1], part.vel[2], energy_index, electron, target_gas);
        }
    }
}
