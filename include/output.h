...
struct GridMeta
{
    std::string type; // "reactconduct", "circular", etc.
    int min_x = 0;
    int min_y = 0;
    int max_x = 0;
    int max_y = 0;
...
class Output 
{
    ...
    void write_metadata(int nx, int ny, int NUM_TS, int write_int, int write_int_phase, double DT,double density, int save_fig, int normscheme, 
    ...
    void write_grid_metadata(const std::vector<GridMeta>& grids_meta);
    void write_grid_mask();
